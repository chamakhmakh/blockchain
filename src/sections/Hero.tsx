...
export const HeroSection = () => {
  ...
  const { scrollYProgress: cuboidScrollYProggress } = useScroll({
    target: cuboidRef,
    offset: ["start end", "end start"],
  });

  const { scrollYProgress: torusScrollYProggress } = useScroll({
    target: torusRef,
    offset: ["start end", "end start"],
  });

  const { scrollYProgress: cubeScrollYProggress } = useScroll({
    target: cubeRef,
    offset: ["start end", "end start"],
  });
  ...
  const cubeRotate = useTransform(cubeScrollYProggress, [0, 1], [100, -45]);
  const torusRotate = useTransform(torusScrollYProggress, [0, 1], [20, -20]);
  const cuboidRotate = useTransform(cuboidScrollYProggress, [0, 1], [20, -20]);

  return (
    <section className="py-24 md:py-52 overflow-x-clip">
      <div className="container">
        <p className="uppercase font-extrabold text-zinc-500 tracking-wide text-center">
          Introducing Blockforge
        </p>
        <h1
          className="font-heading font-black text-5xl md:text-6xl lg:text-7xl 
            text-center mt-4 max-w-3xl mx-auto"
        >
